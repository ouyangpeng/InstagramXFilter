 ...
 *
 * 在色彩理论中，明色调是一种与白色颜色的混合，这减少了暗度。而暗色调是与黑色的混合，这增加了暗度。
 * 色调(Tone)则透过和灰色的混合，或是和明色调和暗色调两者着色的混合。
 * 将颜色和任何的中性色混合(包括黑，灰和白色)可降低彩度或视彩度，同时色相保持不变。
 *
 * 色调不是指颜色的性质，是对一幅绘画作品的整体评价。
 * 一幅绘画作品虽然可能用了多种颜色，但总体有一种色调，是偏蓝或偏红，是偏暖或偏冷等等。
 *
 * 色调(Hue)描述的是整体的颜色效果，但是在HSV/HLS色彩空间中调整效果不是很理想，这里建议切换到YIQ色彩空间进行计算
 */
class GPUImageHueFilter @JvmOverloads constructor(private var hue: Float = 0.0f) :
    GPUImageAdjustFilter(
        NO_FILTER_VERTEX_SHADER, CONTRAST_FRAGMENT_SHADER
    ) {
    private var hueLocation = 0
    override fun onInit() {
        super.onInit()
