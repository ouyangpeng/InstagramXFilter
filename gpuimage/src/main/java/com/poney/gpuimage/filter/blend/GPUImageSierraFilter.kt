...
class GPUImageSierraFilter :
    GPUImageBlendFilter(NO_FILTER_VERTEX_SHADER, readShaderFromRawResource(R.raw.sierra)!!) {
    ...
    override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] =
                loadTexture(GPUImageParams.sContext!!, "filter/sierravignette.png")
            inputTextureHandles[1] = loadTexture(GPUImageParams.sContext!!, "filter/overlaymap.png")
            inputTextureHandles[2] = loadTexture(GPUImageParams.sContext!!, "filter/sierramap.png")
        }
    }
}