...
class GPUImageHudsonFilter :
    GPUImageBlendFilter(NO_FILTER_VERTEX_SHADER, readShaderFromRawResource(R.raw.hudson)!!) {
    ...
    override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] =
                loadTexture(GPUImageParams.sContext!!, "filter/hudsonbackground.png")
            inputTextureHandles[1] = loadTexture(GPUImageParams.sContext!!, "filter/overlaymap.png")
            inputTextureHandles[2] = loadTexture(GPUImageParams.sContext!!, "filter/hudsonmap.png")
        }
    }
}